...
int db_open(char *name)
{
	...
	ret = sqlite3_exec(db, "CREATE TABLE messages(id INTEGER PRIMARY KEY AUTOINCREMENT, time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, key TEXT , value TEXT);", NULL, 0, &zErrMsg);
	if( ret != SQLITE_OK ){
