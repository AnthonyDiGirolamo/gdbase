...
int main(int argc, char* argv[]) {
	...
	if (getenv("MPIRUN_RANK")) {
		temp = getenv("MPIRUN_RANK");
		rank = atoi(temp);
	}
	if (getenv("MPIRUN_NPROCS")) {
		temp = getenv("MPIRUN_NPROCS");
		size = atoi(temp);
	}
	if (getenv("OMPI_MCA_ns_nds_vpid")) {
		temp = getenv("OMPI_MCA_ns_nds_vpid");
		rank = atoi(temp);
	}
	if (getenv("OMPI_MCA_ns_nds_num_procs")) {
