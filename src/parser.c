...
char* get_prefix() {
	if (!parsed) return NULL;
		...
		if (vector_findString(keys, PREFIX) > -1) {
			sprintf(return_string, "%s", (char*) vector_getItem(values, vector_findString(keys, PREFIX)));
...
int parse (char* file_name) {
	...
	int is_a_key;
	int saved_key_value;
	while (line_length > -1) // while not EOF
	{
		is_a_key = FALSE;
		saved_key_value = FALSE;
		line_length = getline(&buffer, &line_size, file_pointer);

		if (line_length > 1) // if not an empty line
		{
			token = strtok(buffer, DELIMITERS);
			if (token[0] != 35) // if first token doesn't start with #
			{
				while (token != NULL) {
					token_length = strlen(token);
					if (token[token_length-1] == 58) // we have a key
					{
					...
					if (token != NULL) // if there is another token
					{
						if (is_a_key) // if the previous token was a key save the pair
						{
							token_length = strlen(token);
					...
					if (!saved_key_value) // we have a prepend
					{
						if (prepend == NULL)
